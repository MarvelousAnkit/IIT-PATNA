/* eslint-disable */

import { HiX } from "react-icons/hi";
import NavLinks from "./components/NavLinks";

import authImg from "assets/img/auth/iitp_logo.png";
import LinkMenu from "./components/LinkMenu";

const Sidebar = ({ open, onClose, routes }) => {
  return (
    <div
      className={`sm:none duration-175 linear fixed !z-50 flex min-h-full flex-col bg-white pb-10 shadow-2xl shadow-white/5 transition-all dark:!bg-navy-800 dark:text-white md:!z-50 lg:!z-50 xl:!z-0 ${open ? "translate-x-0" : "-translate-x-96"
        }`}
    >
      <span
        className="absolute top-4 right-4 block cursor-pointer xl:hidden"
        onClick={onClose}
      >
        <HiX />
      </span>

      <div className={`mx-[56px] mt-[50px] flex flex-col items-center`}>
        <img src={authImg} className="w-14" alt="" />
        <div className="mt-1 ml-1 h-2.5 font-poppins text-[26px] font-bold uppercase text-navy-700 dark:text-white">
          IIT Patna <span className="font-medium">AIS</span>
        </div>
      </div>
      <div className="mt-[58px] mb-7 h-px bg-gray-300 dark:bg-white/30" />
      {/* Nav item */}

      <p className="text-center font-semibold text-lg bg-navy-700 text-gray-200 border-b p-2">{routes.title}</p>

      <ul className="mb-auto pt-1">
        <NavLinks routes={routes} onClose={onClose} />
      </ul>


      {/* Nav item end */}
    </div>
  );
};

export default Sidebar;
